...
## Диагностика завершения процесса

При аварийном завершении CLI после обычного сообщения об ошибке печатает
отдельный безопасный маркер:

```text
OLCRTC_EXIT category=provider_connect code=11
```

Категория и код стабильны, не содержат URL комнаты, ключи или исходный текст
ошибки и подходят для автоматической классификации:

| Код | Категория | Этап |
|---:|---|---|
| 1 | `runtime` | неклассифицированная ошибка выполнения |
| 2 | `config` | чтение или проверка конфигурации |
| 10 | `provider_setup` | создание провайдера или транспорта |
| 11 | `provider_connect` | подключение к провайдеру/Jitsi |
| 12 | `tunnel_setup` | создание multiplexed-сессии туннеля |
| 13 | `handshake` | OLC3 handshake |
| 14 | `peer_identity` | подтверждение identity удалённого peer |
| 15 | `peer_wait` | ожидание удалённого peer |
| 16 | `local_listener` | запуск локального SOCKS listener |

Полный текст ошибки остаётся выше в stderr для локальной диагностики. Для
автоматизации следует разбирать только строку `OLCRTC_EXIT` и принимать лишь
известные категории.
