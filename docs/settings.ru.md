...
| `net.interface` | Необязательное имя сетевого интерфейса ОС, например `en1`. Для Jitsi на поддерживаемых платформах DNS, HTTP/WebSocket signaling и ICE-сокеты привязываются к нему, а Pion собирает кандидатов только с этого интерфейса. Неверное или неактивное имя останавливает запуск. Пустое значение сохраняет обычную маршрутизацию. |
